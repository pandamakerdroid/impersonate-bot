...
with TelegramClient(config['user']['session'], config['user']['api_id'], config['user']['api_hash']) as client:
   ...
   async def handler(event):
      await event.reply('Hey! You have reached out to the automated bot answer, please mind that your message will be disregarded nevertheless..')
   ...
   async def handler(event):
      if(event.chat_id == config['user']['tom']):
         await event.reply('阿綱說要請我們所有人吃帝王蟹, 去台灣得頭等艙往返機票也是阿綱全權負責🙈')

   @client.on(events.NewMessage(pattern='(?i).*(币圈).*(多久了？)'))
   async def handler(event):
      if(event.chat_id == config['user']['tom']): #Tooms group
         await event.reply('見不得人的傢伙, 去死去死去死')
